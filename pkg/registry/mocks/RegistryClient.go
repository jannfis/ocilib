...
import (
	...
	options "github.com/jannfis/ocilib/pkg/options"

	tag "github.com/jannfis/ocilib/pkg/tag"
)
...
// TagInfoFromReferences provides a mock function with given fields: opts, ti, references
func (_m *RegistryClient) TagInfoFromReferences(opts *options.ManifestOptions, ti *tag.TagInfo, references []distribution.Descriptor) (*tag.TagInfo, error) {
	ret := _m.Called(opts, ti, references)

	if len(ret) == 0 {
		panic("no return value specified for TagInfoFromReferences")
	}

	var r0 *tag.TagInfo
	var r1 error
	if rf, ok := ret.Get(0).(func(*options.ManifestOptions, *tag.TagInfo, []distribution.Descriptor) (*tag.TagInfo, error)); ok {
		return rf(opts, ti, references)
	}
	if rf, ok := ret.Get(0).(func(*options.ManifestOptions, *tag.TagInfo, []distribution.Descriptor) *tag.TagInfo); ok {
		r0 = rf(opts, ti, references)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(*options.ManifestOptions, *tag.TagInfo, []distribution.Descriptor) error); ok {
		r1 = rf(opts, ti, references)
	} else {
		r1 = ret.Error(1)
	}
